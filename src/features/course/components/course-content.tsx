...
function getCourseStructure(baseDir: string) {
  const sections = fs.readdirSync(baseDir, { withFileTypes: true })
    ...
    .map((sectionDir) => {
      ...
      console.log(files, 'files in section:', sectionDir.name)
      ...
      return {
        title: sectionDir.name.replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()),
...
export default async function CourseContent() {
  const baseDir = path.join(process.cwd(), 'src/features/course/course-content')
